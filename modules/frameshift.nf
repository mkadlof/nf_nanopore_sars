process frameshift {
    ...
    awk '{split(\$8,a, ";"); split(a[1], DP, "="); if(\$1 != "${REF_GENOME_ID}" || (length(\$5) - length(\$4)) % 3 == 0) print \$0; else if (\$1 == "${REF_GENOME_ID}" && (length(\$5) - length(\$4)) % 3 != 0 && DP[2] >= \$min_cov_frameshift) print \$0}' medaka_and_varscan.vcf >> tmp.vcf

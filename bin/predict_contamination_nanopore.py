...
with open(f'{title}_contamination_summary.txt', 'w') as f:
    if len(dane_sample_fortest) < 20:
        ...
        if pval_sample04 < 0.1 and pval_sample05 < 0.1:
            f.write(f'{title} is not contaminated with other SARS-CoV-2 material\t{pval_sample04}\t{pval_sample05}\n')
        elif pval_sample04 > 0.1 and pval_sample05 >0.1:
            f.write(f'{title} is contaminated with other SARS-CoV-2 material\t{pval_sample04}\t{pval_sample05}\n')
        elif pval_sample04 > 0.1 or pval_sample05 > 0.1:
            f.write(f'{title} might be contaminated with other SARS-CoV-2 material\t{pval_sample04}\t{pval_sample05}\n')
